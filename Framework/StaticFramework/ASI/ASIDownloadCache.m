...
{
    // If we are ignoring cache control headers. only honor a custom max-
    //age
...
- (BOOL)isCachedDataCurrentForRequest:(ASIHTTPRequest *)request
{
    ...
    if ([self shouldRespectCacheControlHeaders]) {
        ...
        //and replaced it with the following simplification:
        
        // Look for X-ASIHTTPRequest-Expires header to see if the content is
        //out of date
        NSNumber *expires = [cachedHeaders objectForKey:@"X-ASIHTTPRequest-Expires"];
        if (expires) {
            if ([[NSDate dateWithTimeIntervalSince1970:[expires doubleValue]]
